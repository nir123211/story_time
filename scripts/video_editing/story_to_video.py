...
def create_video_chunks(story_dir):
    ...
    for line in lines:
        ...
        if 'line.mp4' in os.listdir(os.path.join(story_dir, line)):
            continue
        if 'image.png' in os.listdir(os.path.join(story_dir, line)):
            image = cv.imread(os.path.join(story_dir, line, 'image.png'))
            image = cv.cvtColor(image, cv.COLOR_BGR2RGB)

        with open(os.path.join(story_dir, line, 'line.txt'), 'r') as f:
            line_text = f.read()
        ...
        def make_frame(t):
            ...
            return frame
        dub = (AudioFileClip(os.path.join(story_dir, line, 'line.mp3')).fx(audio_normalize).fx(audio_fadeout, 0.5).
               fx(volumex, 0.8))
        if 'sound.mp3' in os.listdir(line_path):
            try:
                effect = ((AudioFileClip(os.path.join(line_path, 'sound.mp3')).fx(audio_normalize).fx(volumex, 0.2)).
                          fx(audio_fadeout, 1))
            except ZeroDivisionError:
                effect = ((AudioFileClip(os.path.join(line_path, 'sound.mp3')).fx(volumex, 0.2)).
                          fx(audio_fadeout, 1))
        ...
        clip.write_videofile(os.path.join(story_dir, line, 'line.mp4'), fps=25)
